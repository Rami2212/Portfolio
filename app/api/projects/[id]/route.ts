...
export async function PATCH(req: NextRequest, { params }: RouteContext) {
  ...
  if (body?.shortDescription !== undefined)
    updates.shortDescription = String(body.shortDescription || "");

  if (body?.longDescription !== undefined)
    updates.longDescription = String(body.longDescription || "");
  ...
  if (body?.galleryImages !== undefined)
    updates.galleryImages = Array.isArray(body.galleryImages)
      ? body.galleryImages
      : [];

  if (body?.techStack !== undefined)
    updates.techStack = Array.isArray(body.techStack)
      ? body.techStack
      : [];

  if (body?.liveUrl !== undefined)
    updates.liveUrl = String(body.liveUrl || "");
